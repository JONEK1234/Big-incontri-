...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error listening to encounters:", error);
      setIsLoading(false);
    });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("Error listening to special dates:", error);
    });
  ...
  const handleDeleteSpecialDate = async (id: string) => {
    try {
      await deleteDoc(doc(db, "special_dates", id));
    } catch (err) {
  ...
  const handleDeleteLoveMessage = async (id: string) => {
    try {
      await deleteDoc(doc(db, "love_messages", id));
    } catch (err) {
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      ...
      case "gallery":
        return <PhotoGallery encounters={encounters} />;
      case "calendar":
        return (
          <CalendarView
